...
GLuint GLUtils::LoadShader(GLenum shaderType, const char *pSource) {
    ...
    if (shader) {
        LOGD("Create shader successfully.");
...
GLuint GLUtils::CreateProgram(const char *vertexShaderSource, const char *fragShaderSource,
    ...
    GLuint program = 0;
    vertexShaderHandle = LoadShader(GL_VERTEX_SHADER, vertexShaderSource);
    if (!vertexShaderHandle) return program;
    fragShaderHandle = LoadShader(GL_FRAGMENT_SHADER, fragShaderSource);
